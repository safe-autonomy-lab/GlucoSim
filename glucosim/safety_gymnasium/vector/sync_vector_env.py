...
class SafetySyncVectorEnv(VectorEnv):
    ...
    Example:
        ...
        >>> obs, rewards, costs, terminates, truncates, infos = envs.step(actions)
        >>> obs
    ...
    def step(
        ...
        """
        # Handle discrete actions which gymnasium.vector.utils.iterate cannot handle
        
        if isinstance(self.action_space, spaces.Discrete):
            # For discrete actions with single environment, just wrap in a list
            if self.num_envs == 1:
                actions = [actions]
            else:
                # For multiple environments, actions should already be a list/array
                if not hasattr(actions, '__iter__') or len(actions) != self.num_envs:
                    raise ValueError(f"Expected {self.num_envs} actions for discrete action space, got {actions}")
                actions = list(actions)
        else:
            # Use original iterate for other action space types
    ...
    def get_attr(self, name: str, *args: Any, **kwargs: Any) -> tuple[Any, ...]:
